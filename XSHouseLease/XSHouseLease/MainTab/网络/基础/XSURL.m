...
//  XSHouseLease
//
//  Created by heartbeats on 2020/3/20.
 ...
 */
@implementation XSURL
- (instancetype)init{
    self = [super init];
    NSURL *url = [NSURL URLWithString:@"http://www.baidu.com"]; //1
    NSURLRequest *request = [[NSURLRequest alloc]initWithURL:url];
    
    [NSURLConnection sendAsynchronousRequest:request queue:[NSOperationQueue alloc] completionHandler:^(NSURLResponse * _Nullable response, NSData * _Nullable data, NSError * _Nullable connectionError) {
        if (connectionError == nil) {
//            NSLog(@"%@",[[NSString string]initWithData:data encoding:NSUTF8StringEncoding]);
        }
    }];
