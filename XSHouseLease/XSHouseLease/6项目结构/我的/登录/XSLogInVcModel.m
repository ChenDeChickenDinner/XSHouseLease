...
@interface XSLogInVcModel()

@end
@implementation XSLogInVcModel

- (void)setPhone:(NSString *)phone{
    _phone = phone;
    _allowLogin = [self checkallowLogin];
}
- (void)setPictureCheckCode:(NSString *)pictureCheckCode{
    _pictureCheckCode = pictureCheckCode;
    _allowLogin = [self checkallowLogin];

}
- (void)setMessageCheckCode:(NSString *)messageCheckCode{
    _messageCheckCode = messageCheckCode;
    _allowLogin = [self checkallowLogin];
}
- (void)setWaitingMessage:(BOOL)waitingMessage{
    _waitingMessage = waitingMessage;
}
- (BOOL)checkallowLogin{
    if (self.logIStatus == XSUserLogIStatus_MessageCheck) {
        return _phone && _messageCheckCode;
    }else{
//        return _phone && _messageCheckCode && _pictureCheckCode;
        return _phone && _messageCheckCode;
